...
- **LINUX-05**: Config screen (from the tray menu) — add/remove accounts, enter tokens, set GitLab base URLs.
...
- **Open research flag (carried from Phase 1 sign-off):** evaluate `libstray` v0.4.0 production readiness vs. hand-rolling D-Bus SNI before starting the shell. Resolve during `/speckit-plan`.
- **UI hint:** yes. Tray via libstray (SNI over D-Bus, no GTK); windowed UI via GTK4 — kept decoupled per CLAUDE.md.
