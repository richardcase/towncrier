...
must_haves:
  ...
  artifacts:
    - path: ".mise.toml"
      provides: "Local tool version pin"
      contains: "zig"
    - path: "build.zig"
...
<prerequisite_check>
Before executing any task, verify Zig is available via mise:

  mise install     # installs versions declared in .mise.toml (if it exists yet)
  zig version      # must print 0.14.0 or higher

The local .mise.toml (created in Task 1) pins the exact version. On first run before
Task 1 completes, the global mise config provides Zig — either is acceptable.

If Zig is not found at all, STOP and surface this message to the user:
  "Zig is required. Install mise (https://mise.jdx.dev) then run: mise install
   Or install Zig directly from https://ziglang.org/download/"

NOTE on API compatibility: If any build.zig API produces a deprecation error,
...
<task type="auto" tdd="false">
  ...
  <action>
Create three files at the repo root. Do NOT use any deprecated Zig build APIs (no
...
Pin the Zig toolchain version for all contributors:
...
declared, test-c step wired. No extra steps needed in Phase 1.
  ...
  <acceptance_criteria>
    ...
    - `.mise.toml` exists and contains `zig`
